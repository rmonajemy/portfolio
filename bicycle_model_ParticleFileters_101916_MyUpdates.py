...
#  GOAL
    # take measurement of robot wrt landmarks and estimate current state 
    #  current state is x,y, orientation in this case
    # calculation of state using steering angle and distance travelled is not needed
    # and is used to just to verify that particle filters can estime current 
    # state wherever the robot is.
...
class robot:
    ...
    def move(self, motion, tolerance = 0.001): # Do not change the name of this function
        # motion[0] = alpha  (as input)
        # motion[1] = distance moved (as input)
        # calculate new position  (x,y, theta)  = F (old x,y, theta)
...
def get_position(p):  # average of x,y,angle of all particles.  input is a  particle vector consisting of many particles
...
def particle_filter(motions, measurements, N=2000): # I know it's tempting, but don't change N!
    ...
    for t in range(len(motions)):  # do this for each motion
        ...
        for i in range(N):         # (c) resample N of particles giving higher chance to particles that have higher probability factor
            beta += random.random() * 2.0 * mw  #  factor 2.0 is chosen intuitively because it works not in a scientifici way
            while beta > w[index]:
                beta -= w[index]
                index = (index + 1) % N
            p3.append(p[index])
...
###  JUST RESAMPLE ANOTHER TIME TO SEE IF THIS HELP
#        p4 = []
#        index = int(random.random() * N)
#        beta = 0.0
#        mw = max(w)
...
#                index = (index + 1) % N
#            p4.append(p[index])
#        p = p3  # IGNORE 2ND RESAMPLEING
##################################################        
        
    
        print 'estimated position: ',  get_position(p) 
...
##    It will print the robot's last location when calling 
...
#  2.0pi/20  is steering angle which is fixed
#  12.0 is how many units robot moves each time

##
x = generate_ground_truth(motions)  # x[1] = [z1, z2, z3, z4] at each position
                                    # x[0] = robots state (x,,y,orientataion)
  # this is measuremnet of robot wrt landmarka + noise at each location
...
for jj in range(number_of_iterations):
 print 'Ground truth:    ', final_robot[jj]
